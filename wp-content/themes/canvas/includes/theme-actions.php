...
if ( ! function_exists( 'woo_slider' ) ) {
	...
	jQuery(window).load(function() {
		...
		<?php echo "\n"; ?>
		<?php if ( intval( $args['slideSpeed'] ) > 0 ) { ?>args.animationDuration = <?php echo intval( $args['slideSpeed'] ) ?>;<?php } ?>
		<?php echo "\n"; ?>
		...
		<?php echo "\n"; ?>
		<?php if ( $args['autoHeight'] == 'true' ) { ?>
		if ( jQuery( '#loopedSlider .slide' ).length > 1 ) {
			args.useCSS = false;
			args.start = function(slider){ // init the height of the first item on start
				slider.slides.eq(0).css( 'max-width', '100%' ).css( 'height', 'auto' );
	            ...
	            slider.find( '.container' ).height( new_height );                                          
	        },          
	        args.after = function(slider){ // init the height of the next item before slide
	            ...
	            if( new_height != slider.container.height() ) {
	                slider.find( '.container' ).stop().animate({ height: new_height }, 250, 'linear' );
	            }
	        }
	    }
		<?php } ?>
	...
	jQuery(window).resize(function() {  
		if ( jQuery( '#loopedSlider .slide' ).length > 1 ) { 
	    	...
	    	this.id = setTimeout( wooDoResizeSlider, 500 );  
	    }         
	});
	
	function wooDoResizeSlider() {
	    var slider = jQuery( '#loopedSlider .container' );
	    var newHeight = jQuery( 'li.current', slider ).height();      
	    slider.stop().animate({ height: newHeight });   
	} // End wooDoResizeSlider()
	<?php } ?>
	</script>
